...
inline std::string scalar(const flrapidjson::Value&o,const char*k,const char*f=""){if(!o.HasMember(k))return f;const flrapidjson::Value&v=o[k];if(v.IsString())return v.GetString();char b[64];if(v.IsInt()){std::snprintf(b,sizeof(b),"%d",v.GetInt());return b;}if(v.IsNumber()){std::snprintf(b,sizeof(b),"%g",v.GetDouble());return b;}return f;}
